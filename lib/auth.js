 ...
 *     /health, cookie/auth-session import (when CAMOFOX_API_KEY set), and /stop (when CAMOFOX_ADMIN_KEY set).
 ...
 *   - POST /sessions/:userId/cookies and /auth-sessions/* (only when CAMOFOX_API_KEY is also set -- has their own gate)
...
export function accessKeyMiddleware(config) {
  return function accessKeyCheck(req, res, next) {
    ...
    if (config.apiKey && /^\/sessions\/[^/]+\/cookies$/.test(req.path)) return next();
    if (config.apiKey && /^\/tabs\/[^/]+\/cookies$/.test(req.path)) return next();
    if (config.apiKey && /^\/auth-sessions\//.test(req.path)) return next();
